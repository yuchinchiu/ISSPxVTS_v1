# -*- coding: utf-8 -*-
"""
Created on Thu Aug  3 11:37:09 2017
...
def extractDataFromCSV(dataDir, csvDir):
    ...
    def inferSRmapping(RT): 
        ...
        RT = pd.DataFrame(np.transpose(RT.reshape(11,1056)),columns=colNames)
        SRmapping=[0,0,0,0]
        SRmapping[0]=RT.loc[(RT['runId']<'8') & (RT['task']=='1') & (RT['stimCat']=='1') & (RT['sbjACC']=='1'),'response'].astype(int).mean()
        SRmapping[1]=RT.loc[(RT['runId']<'8') & (RT['task']=='1') & (RT['stimCat']=='3') & (RT['sbjACC']=='1'),'response'].astype(int).mean()
        SRmapping[2]=RT.loc[(RT['runId']<'8') & (RT['task']=='2') & (RT['stimCat']=='1') & (RT['sbjACC']=='1'),'response'].astype(int).mean()
        SRmapping[3]=RT.loc[(RT['runId']<'8') & (RT['task']=='2') & (RT['stimCat']=='2') & (RT['sbjACC']=='1'),'response'].astype(int).mean()
